...
$sql = "SELECT FL.fnumber AS FLnumber, pname, B.ID AS bookid, time, B.date,  dairport, dtime, aairport, atime, C.classname AS classtype, capacity, price
            ...
            WHERE (FL.fnumber = C.fnumber) AND (B.fnumber = C.fnumber) AND (classtype = C.classname) AND (FL.aid = AP.aid) AND  B.username = '$username' AND paid = '0'
            ORDER BY time";
...
$rowcount = mysqli_num_rows($result);
    ...
    while($row = mysqli_fetch_array($result)) {
		...
		$sum = mysqli_query($conn,"SELECT SUM(price)
							            ...
							            WHERE B.username = '$username' AND paid = '0' AND flightype = '$oneway'
...
$rowcount = mysqli_num_rows($result);
    ...
    while($row = mysqli_fetch_array($result)) {
        ...
        $sum = mysqli_query($conn,"SELECT SUM(price)
                                        FROM books B, class C
                                        WHERE B.username = '$username' AND paid = '0' AND flightype = '$round'
                                        AND classtype=C.classname AND B.fnumber = C.fnumber");
